...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest (options) {
    ...
    // support options function
    var funArgs = ['headers', 'proxy'];
    for (var i = funArgs.length - 1; i >= 0; i--) {
        var k = funArgs[i];
        var v = options[k];
        if(_.isUndefined(v)) v = this[k];
        if(_.isFunction(v)) {
            ropts[k] = v();
        }
    };
...
Crawler.prototype._onContent = function _onContent (error, options, response, fromCache) {
    ...
    if (error) {
        ...
        if (options.retries) {
            ...
            console.error('Retry end. callback');
            options.callback(error);
        }
