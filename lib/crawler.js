...
Crawler.prototype.init = function init (options) {
    ...
    self.globalOnlyOptions = ['maxConnections', 'rateLimits', 'priorityRange', 'homogeneous', 'skipDuplicates', 'rotateUA', 'onDrain', 'onLimiterChange'];
    ...
    self.on('_release', self._release.bind(self));
};
...
Crawler.prototype._pushToQueue = function _pushToQueue (options) {
    ...
    if (options.skipDuplicates && self.seen.exists(options)) {
        return self.emit('_release', options);
    }
