...
Crawler.prototype.init = function init (options) {
    ...
    self.once('pool:drain', function(options) {
        if (options.onDrain) {
            options.onDrain();
        }
    })
...
Crawler.prototype._inject = function _inject (response, options, callback) {
    ...
    if (options.jQuery === "cheerio" || options.jQuery.name === 'cheerio') {
    ...
    if (options.jQuery === "jsdom" || options.jQuery.name === 'jsdom' || options.jQuery === true) {
        // TODO WINDOWS env
        var scriptLocation = options.jQuery.options.url  || path.resolve(__dirname,"../vendor/jquery-1.8.3.min.js");
...
Crawler.prototype._makeRequest = function _makeRequest (options) {
    var self = this;
...
Crawler.prototype._executeRequest = function _executeRequest (options) {
    var self = this;
