...
Crawler.prototype._inject = function _inject (response, options, callback) {
    ...
    else if (options.jQuery === 'jsdom' || options.jQuery.name === 'jsdom' || options.jQuery === true) {
        // TODO WINDOWS env
        var scriptLocation = options.jQueryUrl || path.resolve(__dirname,'../vendor/jquery-1.8.3.min.js');
