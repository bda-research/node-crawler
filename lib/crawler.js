...
Crawler.prototype.generateHttp2RequestLine = function (options) {
	...
	requestLine[':method'] = options.method || 'POST';
	requestLine[':path'] = urlObj.pathname;
	requestLine[':scheme'] = urlObj.protocol.replace(':', '');
	requestLine[':authority'] = urlObj.hostname;

	return requestLine;
};

Crawler.prototype.generateHttp2RequestBody = function (options) {
	if (options.headers[':method'] === 'GET') return null;
...
Crawler.prototype._clearHttp2Session = function _clearHttp2Session(targetHost) {
	const self = this;
	log('debug', `==================clear http 2 connection=================`);
	Object.keys(self.http2Connections).forEach(hostName => {
		self._closeAndDeleteHttp2Session(hostName);
...
Crawler.prototype._closeAndDeleteHttp2Session = function _closeAndDeleteHttp2Session(targetHost) {
	const self = this;
	if(self.http2Connections[targetHost]) {
		self.http2Connections[targetHost].close();
		delete self.http2Connections[targetHost];
	}
};
...
Crawler.prototype._buildHttpRequest = function _buildHTTPRequest(options) {
	...
	var doRequest = function (err) {
		...
		//do http2.* request 
		if (ropts.http2) {
			//append http2 request line to http2 headers
			ropts.headers = Object.assign(ropts.headers, self.generateHttp2RequestLine(ropts));
			const requestBody = self.generateHttp2RequestBody(ropts);
			const response = {};
			response['headers'] = {};
			const chunks = [];
			...
			if (!self.http2Connections[targetHost] || self.http2Connections[targetHost].destroyed) {
				self.http2Connections[targetHost] = http2.connect(targetHost);

				log('debug',`connect to a new ${targetHost}`);
				self.http2Connections[targetHost].on('error', (err) => {
					http2Error = err;

					if(self.http2Connections[targetHost]) self._closeAndDeleteHttp2Session(targetHost);
				});

				self.http2Connections[targetHost].on('frameError', (err) => {
					http2Error = err;
				});

				self.http2Connections[targetHost].on('goaway', () => {
					log('debug',`${targetHost} connection goaway`);
				});

				self.http2Connections[targetHost].on('connect', () => {
					log('debug',`${targetHost} connection init`);
				});

				self.http2Connections[targetHost].on('timeout', (err) => {
					http2Error = err;
				});

				self.http2Connections[targetHost].on('close', () => {
					log('debug',`${targetHost} connection closed`);
					if(self.http2Connections[targetHost])  self._closeAndDeleteHttp2Session(targetHost);
				});
			}

			const currentHttp2Session = self.http2Connections[targetHost];
			let req;

			try {
				req = currentHttp2Session.request(ropts.headers);
			} catch (e) {
			...
			req.on("response", (headers, flags) => {
				...
				response['request'] = {};
				response['request']['uri'] = options.uri;
			...
			req.on('error', (err) => {	
				log('error', `inside http2 stream, got error`);
				http2Error = err;
			})
			...
			if (self.options.timeout !== undefined && self.options.timeout > 0) {
