...
function Crawler(options) {
    ...
    var release = function(opts) {
        queueItemSize--;
        if (opts._poolReference) {
            self.pool.release(opts._poolReference);
        }

        // Pool stats are behaving weird - have to implement our own counter
        if (queueItemSize+plannedQueueCallsCount === 0) {
    ...
    self.queue = function(toQueue) {

        //Did I get a list ? Queue all the URLs.
        if (_.isArray(toQueue)) {
            var queueLength = toQueue.length
            for (var i = 0; i < queueLength; i++) {
                self.queue(toQueue[i]);
            }
        // Did I get a simple string instead, convert to object and queue
        } else if (_.isString(toQueue)) {
            self._addToQueue({
                uri: toQueue
            });
        } else {
            self._addToQueue(toQueue);
        }
    };

    self._addToQueue = function(item) {
        queueItemSize++;

        _.defaults(item, self.options);

        // Remove all the global options from our item
        // TODO we are doing this for every _addToQueue, find a way to avoid this
        _.each(globalOnlyOptions, function(globalOnlyOption) {
            delete item[globalOnlyOption];
        });

        // If duplicate skipping is enabled, avoid queueing entirely for URLs we already crawled
        if (item.skipDuplicates && self.cache[item.uri]) {
            return release(item);
        }
        ...
        self.pool.acquire(function(error, poolReference) {
            ...
            item._poolReference = poolReference;

            //Static HTML was given, skip request
            if (item.html) {
                self.onContent(null, item, {body:item.html}, false);
            } else if (typeof item.uri === "function") {
                item.uri( function(uri) {
                    item.uri = uri;
                    self.request(item);
                });
            } else {
