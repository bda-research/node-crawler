...
Crawler.prototype._buildHttp2Session = function _buildHttp2Session(targetHost) {
	...
	self.http2Connections[targetHost].on('timeout', () => {
		log('debug', `Http2 session${targetHost} connection timeout`);
	});
...
Crawler.prototype._http2request = function _http2request(ropts, options) {
	...
	if (!self.http2Connections[targetHost] || self.http2Connections[targetHost].destroyed) self._buildHttp2Session(targetHost);
