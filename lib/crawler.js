...
Crawler.prototype.init = function init (options) {
    ...
    self.globalOnlyOptions = ['maxConnections', 'rateLimits', 'priorityRange', 'homogeneous', 'skipDuplicates', 'rotateUA'];

    self.limiters = new Bottleneck.Cluster(self.options.maxConnections,self.options.rateLimits,self.options.priorityRange, self.options.priority, self.options.homogeneous);
	...
	callback(null, response, options, $);
        ...
        readJqueryUrl(scriptLocation, function(err, jquery) {
            try {
                ...
                options.callback(e,{options},null, options.release);
            }
        });
    }
...
Crawler.prototype._onContent = function _onContent (error, options, response, fromCache) {
    ...
    if (error) {
		...
		options.release();
            ...
            options.callback(error,{options:options},null,options.release);
        }
	...
	try{
            ...
            log("error",e);
            if(options.callback){
		options.callback(e,{options:options},null,options.release);
            }
            return
	}
    }
	...
	options.callback(null,response,null,options.release);
        return
    }
    
    if (!options.html && !typeis(response, ['html'])){
	log("warn","response body is not HTML, ignore injecting");
	options.callback(null,response,null,options.release);
	return
    }
